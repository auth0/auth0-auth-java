 ...
 *
 * <p>This class binds Spring Boot configuration properties prefixed with {@code auth0} to provide
 ...
 * </pre>
 *
 * <p>When {@code domains} is configured, the SDK validates the token's {@code iss} claim against
 * all listed domains and performs OIDC discovery for the matching issuer. The built-in in-memory
 * cache handles caching of discovery metadata and JWKS providers automatically.
 *
...
public class Auth0Properties {
   ...
   *
   * <p>When configured, tokens whose {@code iss} claim matches any of these domains will be
   * accepted. Cannot be used together with a dynamic {@code domainsResolver}. Can coexist with
   * {@link #domain} — if both are set, this list takes precedence for token validation. Example:
   *
   * <pre>
   * auth0:
   *   domains:
  ...
  /**
   * Maximum number of entries in the unified in-memory cache (OIDC discovery + JWKS providers).
   * Default: 100.
   */
  private Integer cacheMaxEntries;

  /**
   * TTL in seconds for cached entries (OIDC discovery + JWKS providers). Default: 600 (10 minutes).
   */
   ...
   *
   * @return the configured max entries, or {@code null} if not set (uses default of 100)
   */
   ...
   *
   * @return the configured TTL in seconds, or {@code null} if not set (uses default of 600)
   */
